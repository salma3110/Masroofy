...
def setup(request):
    if request.method == 'POST':
        ...
        # Deactivate any existing cycle
        BudgetCycle.objects.filter(is_active=True).update(is_active=False)

        # Create new cycle using set_allowance logic
        cycle = BudgetCycle(start_date=sd, end_date=ed, is_active=True)
...
def change_budget(request):
    ...
    if request.method == 'POST':
        ...
        cycle.set_allowance(allowance)  # validates, recalculates, saves
